...
export class AuthService {
  ...
  private SIGNUP_URL='http://localhost:8080/api/v1/auth/signup';
  ...
  private emailId: number | null = null;
    ...
    return this.httpClient.post<any>(this.LOGIN_URL, {username, password}).pipe(
      tap(response => {
        if(response.token){
          ...
          this.setCartId(response.customer.cartId)
  ...
  signup(username: string, password: string, email: string, direccion:string): Observable<any> {
    return this.httpClient.post<any>(this.SIGNUP_URL, { username, password, email, direccion }).pipe(
      tap(response => {
        if (response.token) {
          console.log(response.token);
          this.setToken(response.token);
  ...
  setCartId(cartId: number) {
    this.cartId = cartId; // Método para establecer el cartId
  }
