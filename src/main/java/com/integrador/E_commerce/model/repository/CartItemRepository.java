...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
	@Query("SELECT ci.producto FROM CartItem ci WHERE ci.producto.id = ?1")
	Producto findProductById(Long productId);

//	@Query("SELECT ci.cart FROM CartItem ci WHERE ci.product.id = ?1")
//	List<Cart> findCartsByProductId(Long productId);

	@Query("SELECT ci FROM CartItem ci WHERE ci.cart.id = ?1 AND ci.producto.id = ?2")
...
//	Order findOrderByEmailAndOrderId(String email, Integer orderId);
	...
	@Modifying
	@Query("DELETE FROM CartItem ci WHERE ci.cart.id = ?1 AND ci.producto.id = ?2")
